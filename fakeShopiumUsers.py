...
def generate_users(db,nbre_users):
    pays = ["kairouan", "sousse", "monastir", "sfax", "tunis", "kasserine", "tozeur", "gafsa", "gabes", "benzart",
            "tataouine"]
    ...
    for i in range(nbre_users):
        amis = []
        friendRequest = []
        sendRequest = []
        for y in range(random.randint(1, 4)):
            amis.append({"_id": fake.uuid4(),
                         "nom": fake.name(),
                         "prenom": fake.name()})

        for x in range(random.randint(1, 4)):
            friendRequest.append({"_id": fake.uuid4(),
                                  "nom": fake.name(),
                                  "prenom": fake.name()})

        for z in range(random.randint(1, 4)):
            sendRequest.append({"_id": fake.uuid4(),
                                "nom": fake.name(),
                                "prenom": fake.name()})
        ...
        db.usersData.insert_one(userobj)
        print(userobj)